...
export const fragmentShader = `
  ...
  // Calculate UVs to mimic object-fit: cover
  ...
  void main() {
    ...
    // Mix original and distorted based on edge factor to keep borders clean
    uv.x = mix(originalX, distortedX, edgeFactor);

    float distortionFactor = uv.x - originalX;
    float parallaxDirection = -sign(0.5 - uMouse.x);

    // Calculate parallax based on mouse position and distortion
    ...
    // Clamp out-of-bounds UVs to prevent glitches (per your screenshot logic)
